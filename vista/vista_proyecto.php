...
class vistaProyecto{
    ...
    function proyecto($proyecto){
        ...
        $this->contenido        = str_replace('<!-- {{id}} -->', $proyecto->getId(), $this->contenido);
        $this->contenido        = str_replace('<!-- {{nombre}} -->', $proyecto->getNombre(), $this->contenido);
        $this->contenido        = str_replace('<!-- {{frecuencia}} -->', $proyecto->getFrecuencia(), $this->contenido);
        $this->contenido        = str_replace('<!-- {{cantidad de participantes}} -->', $proyecto->getCantidadParticipantes(), $this->contenido);
        $this->contenido        = str_replace('<!-- {{accion}} -->', 'PONER ACCION', $this->contenido);
        $this->contenido        = str_replace('<!-- {{comentarios}} -->', $proyecto->getComentarios(), $this->contenido);
        $this->contenido        = str_replace('<!-- {{leyenda}} -->', $proyecto->getLeyenda(), $this->contenido);
        ...
        foreach($proyecto->getParticipantes() as $participante){
            ...
            $textAux        = str_replace('<!-- {{nombre}} -->', $participante->getNombre(), $textAux);
